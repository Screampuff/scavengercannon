...
function ENT:SetupDataTables()
	self:DTVar("Float",0,"LastDamaged")
end
...
function ENT:Think()
	...
	if IsValid(parent) || (self.Alpha != 1) then
		self:SetMoveType(MOVETYPE_NONE)
		self:SetSolid(SOLID_NONE)
		self.Alpha = self.Alpha-FrameTime()
	end
	if SERVER && ((self.Alpha < 0) || !IsValid(self.Weapon)) then
		self:Remove()
	end
end
...
if CLIENT then
	...
	function ENT:Draw()
		local damageglow = math.Clamp(1-(CurTime()-self.dt.LastDamaged),0,1)
