	...
	if CLIENT then
		...
		if not self.Hit then
			...
			for _,v in ipairs(ents.FindInSphere(self:GetPos(),512)) do -- check all enemies within range, add them to the enemies table if they aren't there already
				...
				if IsValid(v) and (v:IsNPC() or (v:IsPlayer() and v:Alive())) and not v:IsFriendlyToPlayer(self.Owner) and v ~= self.Owner and not table.HasValue(self.enemies,v) then
